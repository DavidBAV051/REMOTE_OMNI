...
#define PIN_M3_ENA_A    16U // P1_16
#define PORT_M3_ENA_A   1U
#define PIN_M3_ENA_B    17U // P1_17
#define PORT_M3_ENA_B   1U

// --- Motor 4 (PWM on kPWM_Module_2, kPWM_PwmA - P2_2) ---
#define PIN_M4_ENA_A    5U // P1_8
#define PORT_M4_ENA_A   4U
#define PIN_M4_ENA_B    7U // P0_7
...
#define M2_ADC_PORT     4U
#define M2_ADC_PIN      15U
#define M2_ADC_CHANNEL  1U
#define M2_ADC_CMD_ID   2U

// Motor 3 Current: P0_17 (ADC0 Channel 9 Side A)
#define M3_ADC_PORT     4U
#define M3_ADC_PIN      12U
#define M3_ADC_CHANNEL  5U
#define M3_ADC_CMD_ID   3U

// Motor 4 Current: P0_18 (ADC0 Channel 10 Side A)
#define M4_ADC_PORT     4U
#define M4_ADC_PIN      16U
#define M4_ADC_CHANNEL  6U
...
//ROBOT FUNCTIONS
float dimension_rotation(uint8_t raw_byte);
float dimension_speed(uint8_t raw_byte);
...
void TIMER_0(void){
    /* Trigger the telemetry packet sending */
    Robot_SendTelemetry();
}
...
void PID_TIMER(void){
	...
	ROBOT_compute_kinematics(&ROBOT);
	pid_compute(&M1);
	pid_compute(&M2);
	pid_compute(&M3);
	pid_compute(&M4);
...
int main(void)
{
	...
	/* 3. Initialize SPI Driver BEFORE starting timers */
	ESP_SPI_Init(LPSPI1, LPSPI_MASTER_CLK_FREQ, EXAMPLE_LPSPI_MASTER_PCS_FOR_INIT);

	/* 4. Enable SPI Interrupts in NVIC */
...
void init_hardware(void){
	...
	INPUTMUX->CTIMER0CAP0 = INPUTMUX_CTIMER0CAP0_INP(16U); // M1 (P4_0) -> Cap 16
	INPUTMUX->CTIMER0CAP1 = INPUTMUX_CTIMER0CAP1_INP(17U); // M2 (P4_1) -> Cap 17
	INPUTMUX->CTIMER0CAP2 = INPUTMUX_CTIMER0CAP2_INP(0U);  // M3 (P0_8)-> Cap 0
	INPUTMUX->CTIMER0CAP3 = INPUTMUX_CTIMER0CAP3_INP(1U);  // M4 (P0_9)-> Cap 1
...
/**
 * @brief Processes a raw byte (0-100), ignores the last 2 bits,
 * and scales it to a float between -1.0 and 1.0.
 */
float dimension_speed(uint8_t raw_byte)
{
    // 1. Mask the last two bits (0xFC is 1111 1100 in binary)
    // This effectively rounds the number down to the nearest multiple of 4.
    uint8_t val = (raw_byte & 0xFC);

    // 2. Scale the value
    // We cast to float to prevent integer division truncation.
    // Formula: (Value / 50.0) - 1.0
    float scaled_result = ((float)val / 50.0f) - 1.0f;

    // Optional: Safety clamp
    // Since uint8_t can theoretically go up to 255, if the input
    // accidentally exceeds 100, the result would exceed 1.0.
    if (scaled_result > 1.0f) {
        scaled_result = 1.0f;
    }
...
/**
 * @brief Processes a raw byte (0-100), ignores the last 2 bits,
 * and scales it to a float between -0.01 and 0.01.
 */
float dimension_rotation(uint8_t raw_byte)
{
    // 1. Mask the last two bits (0xFC)
    // Rounds down to the nearest multiple of 4
    uint8_t val = (raw_byte & 0xFC);

    // 2. Scale the value
    // Formula: (Value / 5000.0) - 0.01
    // Range [0, 100] becomes [-0.01, 0.01]
    float scaled_result = ((float)val / 5000.0f) - 0.01f;

    // Optional: Safety clamp
    // Ensures we don't exceed 0.01 if input > 100
    if (scaled_result > 0.01f) {
        scaled_result = 0.01f;
    }
